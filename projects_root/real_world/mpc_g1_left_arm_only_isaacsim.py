...
# Add the examples/isaac_sim directory to sys.path to import helper
# Assuming this script is in projects_root/real_world
# and curobo is in ../../curobo
# We need to find where curobo/examples/isaac_sim is relative to this file
# This logic assumes the standard structure:
# projects_root/real_world/mpc_g1_left_arm_only_isaacsim.py
# curobo/examples/isaac_sim/helper.py

# Go up two levels from this file to get to the root of the repo
repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
curobo_examples_path = os.path.join(repo_root, "curobo", "examples", "isaac_sim")

if os.path.exists(curobo_examples_path):
    sys.path.append(curobo_examples_path)
else:
    print(f"Warning: Could not find curobo examples path at {curobo_examples_path}")
    # Fallback to try finding it via relative path if needed or assume installed
...
parser.add_argument("--robot", type=str, default="g1_feb26/left_arm_only.yml", help="robot configuration to load")
...
try:
    ...
    print("Could not import helper. Make sure curobo/examples/isaac_sim is in python path.")
    sys.exit(1)
...
def main():
    ...
    while simulation_app.is_running():
        ...
        if step_index <= 10:
            # my_world.reset()
            # robot._articulation_view.initialize() # Might need check if initialized
        ...
        if past_pose is None:
            ...
            past_pose = cube_position # Initialize to current to avoid immediate jump check issue
        ...
        if sim_js is None:
            ...
            continue
        # js_names = robot.dof_names # Unused
