"""
version to run in docker container

date: 2025-07-04

# run $omni_python /pkgs/curobo/examples/isaac_sim/realsense_mpc.py
...
def create_colored_material(path, color=(0.0, 1.0, 0.0)):
    ...
    # ✅ Create the surface output explicitly
    surface_output = shader.CreateOutput("surface", Sdf.ValueTypeNames.Token)

    # ✅ Then connect material to the shader's surface output
...
if __name__ == "__main__":
    ...
    target_material = create_colored_material("/World/looks/t",  color=(0.0, 1.0, 0.0))
 # DeformableMaterial("/World/looks/t")# , color=np.array([0, 1, 0])) # OmniPBR
    target_material_2 = create_colored_material("/World/looks/t",  color=(0.0, 1.0, 0.0))
 # DeformableMaterial("/World/looks/t2") #, color=np.array([0, 1, 0])) # OmniPBR
    if not args.waypoints:
        ...
        UsdShade.MaterialBindingAPI(target.prim).Bind(target_material)
    ...
    UsdShade.MaterialBindingAPI(target_2.prim).Bind(target_material_2)
