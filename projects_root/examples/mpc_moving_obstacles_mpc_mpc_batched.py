...
LOAD_ASSETS_MANUALLY = False # Turn on if you want to "drag and drop" assets to the stage manually. Turn otherwise because it takes longer to load the assets.
...
def main():
    ...
    while simulation_app.is_running():
        ...
        # if LOAD_ASSETS_MANUALLY: # "drag and drop" (less frequent updates because it takes longer to load the assets)
        #     for i in range(len(robots)):
        #         if t_idx % 200 == 0:
        #             print("Reading a new world model from the stage and updating the current world model with it")
        #             robots[i].reset_world_model(usd_help, ignore_substrings)
        #             new_cchecker = robots[i].get_cchecker()
        #             for obstacle in env_obstacles:
        #                 obstacle.re_register_cchecker(i, new_cchecker)
