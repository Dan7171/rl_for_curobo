...
def main():
    ...
    if world_cfg_table.cuboid and len(world_cfg_table.cuboid) > 0:
        world_cfg_table.cuboid[0].pose[2] -= 0.02
    world_cfg1 = WorldConfig.from_dict(
        load_yaml(join_path(get_world_configs_path(), "collision_table.yml"))
    ).get_mesh_world()
    if world_cfg1.mesh and len(world_cfg1.mesh) > 0:
    ...
    while simulation_app.is_running():
        ...
        if not world_initialized and step_index > 30:
            ...
            # Update motion_gen world collision checker with the initialized world
            motion_gen.update_world(collision_world)
            # Set the collision checker reference in the wrapper
            world_wrapper.set_collision_checker(motion_gen.world_coll_checker)
        ...
        if not args.reactive:
            if cu_js.velocity is not None:
                cu_js.velocity *= 0.0
            if cu_js.acceleration is not None:
        ...
        if (
            ...
            succ = result.success.item() if result.success is not None else False
            if num_targets == 1:
