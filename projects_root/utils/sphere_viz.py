...
def plot_spheres(spheres: torch.Tensor):
    """
    spheres: torch.Tensor, shape (n_spheres, 4)
    spheres[:, 0]: x coordinates
    spheres[:, 1]: y coordinates
    spheres[:, 2]: z coordinates
    spheres[:, 3]: radii
    """
    spheres = spheres.cpu().numpy()
    # filter out spheres with radius =< 0
    spheres = spheres[spheres[:, 3] > 0]
    fig = plt.figure(); ax = fig.add_subplot(111, projection='3d'); [ax.scatter(spheres[i, 0], spheres[i, 1], spheres[i, 2], s=spheres[i, 3]*1000, alpha=0.6) for i in range(spheres.shape[0])]; ax.set_xlabel('X'); ax.set_ylabel('Y'); ax.set_zlabel('Z'); plt.show()
    plt.savefig(f"tmp_spheres.png")
