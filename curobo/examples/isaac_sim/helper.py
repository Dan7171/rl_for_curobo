...
def add_robot_to_scene(
    ...
    # ----------------------------------------------------------------------------------
    # 1) OPTION A: LOAD FROM A PRE-AUTHORED USD
    # ----------------------------------------------------------------------------------
    if load_from_usd and robot_config["kinematics"].get("usd_path", None) is not None:
        from omni import usd as omni_usd
        ...
        usd_file = join_path(asset_path, robot_config["kinematics"]["usd_path"])

        # Author a new prim in the stage and reference the USD asset
        root_prim_path = (
            str(my_world.scene.stage.GetDefaultPrim().GetPath()) + "/" + robot_name
        )
        root_prim = my_world.scene.stage.DefinePrim(root_prim_path, "Xform")
    ...
    # ----------------------------------------------------------------------------------
        ...
        if ISAAC_SIM_45:
            ...
            dest_path = join_path(
                robot_path, get_filename(filename, remove_extension=True) + "_temp.usd"
            )

            result, robot_path_tmp = omni.kit.commands.execute(
                "URDFParseAndImportFile",
                urdf_path="{}/{}".format(robot_path, filename),
                import_config=import_config,
                dest_path=dest_path,
            )
            prim_path = omni.usd.get_stage_next_free_path(
                my_world.scene.stage,
                str(my_world.scene.stage.GetDefaultPrim().GetPath()) + robot_path_tmp,
                False,
            )
