...
def main():
    ...
    parser.add_argument("--steps", type=int, default=30000, help="Simulationsteps (and frames to record)")
    ...
    if args.video:
        ...
        if args.async_mode:
            ...
            loop.run_until_complete(record_video_async(args.steps, args.output_dir))
            loop.close()
        else:
            # Use sync recording
            record_video_sync(args.steps, args.output_dir)
    else:
        ...
        try:
            while step_count < args.steps:
                ...
                step_count += 1
                if step_count % 100 == 0:
                    print(f"Simulation step: {step_count}/{args.steps}")
